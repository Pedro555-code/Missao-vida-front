...
import { cloneAndAddClass } from "../form"
...
>(({ className, inset, children, ...props }, ref) => (
  <DropdownMenuPrimitive.SubTrigger
    ref={ref}
    className={cloneAndAddClass(
      "flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none focus:bg-zinc-100 data-[state=open]:bg-zinc-100 dark:focus:bg-zinc-800 dark:data-[state=open]:bg-zinc-800" + inset ? "pl-8" : '',
      className
    )}
    {...props}
  >
...
>(({ className, ...props }, ref) => (
  <DropdownMenuPrimitive.Separator
    ref={ref}
    className={cloneAndAddClass("-mx-1 my-1 h-px bg-zinc-100 dark:bg-zinc-800", className)}
    {...props}
  />
))
...
const DropdownMenuShortcut = ({
  ...
  return (
    <span
      className={cloneAndAddClass("ml-auto text-xs tracking-widest opacity-60", className)}
      {...props}
    />
  )
}
